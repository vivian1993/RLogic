...
class Dataset(object):
    def __init__(self, data_root, inv=False):
        ...
        if inv :
            fact_path += '.inv'
        self.rdf_data_ = self.load_data_(fact_path, train_path, valid_path, test_path)
    ...
    def load_data_(self, fact_path, train_path, valid_path, test_path):
    ...
    while len(stack) > 0:
        ...
        if rule_head_rel and cur_t != anchor_t:
            ...
            if PRINT:
                ...
                print('rule:\n{}\n'.format(rule))
        elif rule_head_rel == False and random.random() > 0.95:
            rule = cur_r + '-' + "None"
            rule_seq.append(rule)
            if (cur_h, r_, t_) not in record:
                record.append((cur_h, r_, t_))
